...
const live2d_settings = {
    ...
    'screenshotCaptureName': 'model.png',   // 看板娘截图文件名，例如 'live2d.png'
}
// 模型列表
const live2d_models = [
	{
...
function initModel() {
    ...
    if (!live2d_settings.showToolMenu) $$('.l2d-tools').classList.add('hide');
    if (!live2d_settings.canCloseLive2d) $$('.l2d-tools .icon-cross').classList.add('hide');
    if (!live2d_settings.canSwitchModel) $$('.l2d-tools .icon-next').classList.add('hide');
    if (!live2d_settings.canSwitchHitokoto) $$('.l2d-tools .icon-message').classList.add('hide');
    if (!live2d_settings.canTakeScreenshot) $$('.l2d-tools .icon-camera').classList.add('hide');
    ...
    $$('.l2d-tools .icon-cross').addEventListener('click', () => {
...
function loadModel(modelName) {
    ...
    if (modelVersion === 2) {
        ...
        window.live2dv2.load(live2dId2, `${live2d_settings.modelUrl}/${modelName}/Full/${modelName.substring(0,3)}_${modelName.substring(3).split('/')[0].toLowerCase()}_${modelName.split('/')[1].toLowerCase()}.model.json`, 1);
...
function loadTipsMessage(result) {
    ...
    $$('.l2d-tools .icon-message').addEventListener('click', () => showHitokoto());
}
